...
class Objective(object):
    def __init__(self, config_file, specs = None, verbose=False):
        ...
        if specs == None:
            ...
            self.adj, self.features, self.y_train, self.y_val, self.y_test, self.train_mask, self.val_mask, self.test_mask = load_split_data(configs['dataset'], data_dir='/mnt/_default/datasets/kFold',specs=specs)
...
for fold in range(1,5+1):
    for split in range(1,2+1):
        ...
        objective = Objective(sys.argv[1], specs = 'Fold_%d/%d'%(fold,split), verbose = optuna_params_dict['verbose'])
        ...
        print("Best Hyperparameters Fold_%d/%d"%(fold,split))
        for key, value in trial.params.items():
        ...
        print_and_log("\n\nBest Metrics Fold_%d/%d"%(fold,split), log_file)
        ...
        metrics.append([trial.user_attrs['Train Accuracy'], trial.user_attrs['Val Accuracy'], trial.user_attrs['Test Accuracy'],trial.user_attrs['Train Loss'],trial.user_attrs['Val Loss'],trial.user_attrs['Test Loss']])
        ...
        df = pd.DataFrame(metrics_np, columns=['Train Accuracy','Val Accuracy','Test Accuracy','Train Loss','Val Loss','Test Loss'])
        df.to_csv(objective.output_dir+'/split_metrics.csv',index=False)        
        print_and_log("\n\nBest Average Metrics using %d splits"%metrics_np.shape[0], log_file)
        print_and_log('Train Accuracy   - %0.04f (+-%0.06f)'%(metrics_mean[0],metrics_std[0]), log_file)
        print_and_log('Train Loss       - %0.04f (+-%0.06f)'%(metrics_mean[3],metrics_std[3]), log_file)
        print_and_log('Val Accuracy     - %0.04f (+-%0.06f)'%(metrics_mean[1],metrics_std[1]), log_file)
        print_and_log('Val Loss         - %0.04f (+-%0.06f)'%(metrics_mean[4],metrics_std[4]), log_file)
        print_and_log('Test Accuracy    - %0.04f (+-%0.06f)'%(metrics_mean[2],metrics_std[2]), log_file)
        print_and_log('Test Loss        - %0.04f (+-%0.06f)'%(metrics_mean[5],metrics_std[5]), log_file)
